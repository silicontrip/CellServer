...
for ff in dt:
	...
	tsl = ts - 5000
	tsu = ts + 5000
	query =   {"$or" : [ {"data.points": { "$all" :  [ {"$elemMatch" : llE6[0]}, {"$elemMatch" : llE6[1]}]} }, {"data.points": { "$all" :  [ {"$elemMatch" : llE6[1]}, {"$elemMatch" : llE6[2]}]} }, {"data.points": { "$all" :  [ {"$elemMatch" : llE6[2]}, {"$elemMatch" : llE6[0]}]} } ]}
	#print query
	res= ingresslog.find(query,None)
	oid=[]	
	mu=[]	
